...
?>
  ...
  /*$liste_commande = $pdo->query("SELECT *
    ...
    $liste_commande = $pdo->query("SELECT *, commande.prix AS prix_commande
    FROM commande
    LEFT JOIN produit ON  commande.id_produit = produit.id_produit
    LEFT JOIN salle ON  produit.id_salle = salle.id_salle
    LEFT JOIN membre ON  commande.id_membre = membre.id_membre ORDER BY id_commande");
      ...
      echo '  <thead class="thead-dark">
       ...
       while($ligne = $liste_commande->fetch(PDO::FETCH_ASSOC)) { 
        ...
        echo '<td>' . htmlspecialchars($ligne['prix_commande'], ENT_QUOTES, 'UTF-8') . ' € </td>';
